...
struct CliArgs {
    ...
    // ==================== Fault-Driven Logging ====================
    /// Enable per-request fault-driven logging. When enabled, every
    /// dispatched request's path is recorded into a per-bootstrap_room
    /// ring buffer; when this router (or a Python peer notifying us)
    /// observes an error, the room's events are dumped asynchronously
    /// to --fault-log-dir. Off by default; behavior is identical to
    /// upstream when not set.
    #[arg(long, default_value_t = false, help_heading = "Fault-Driven Log")]
    enable_fault_driven_log: bool,

    /// Directory where fault-driven logs are persisted (one JSON per
    /// failed request, partitioned by date).
    #[arg(
        long,
        default_value = "/tmp/sglang_fault_log",
        help_heading = "Fault-Driven Log"
    )]
    fault_log_dir: String,

    /// Shared secret used to authenticate /fault-log/notify requests
    /// across router/prefill/decode. Empty = auth disabled (use only
    /// in trusted single-tenant networks).
    #[arg(long, default_value = "", help_heading = "Fault-Driven Log")]
    fault_log_token: String,

    /// Externally-reachable base URL of THIS router, written into the
    /// X-Sgl-Router-Fault-Log-Url header on dispatch so prefill/decode
    /// know where to send notifies. Auto-derived from --host:--port if
    /// left empty.
    #[arg(long, default_value = "", help_heading = "Fault-Driven Log")]
    fault_log_self_url: String,

    /// Optional case-insensitive substring whitelist for error
    /// messages / status codes. Empty = match all errors.
    #[arg(long, num_args = 0.., help_heading = "Fault-Driven Log")]
    fault_log_error_whitelist: Vec<String>,

    /// Cap on rooms tracked in the per-process ring buffer.
    #[arg(long, default_value_t = 5000, help_heading = "Fault-Driven Log")]
    fault_log_max_rooms: usize,

    /// Cap on events kept per room.
    #[arg(long, default_value_t = 200, help_heading = "Fault-Driven Log")]
    fault_log_max_events_per_room: usize,

    /// Idle TTL (seconds) before a room is GC'd.
    #[arg(long, default_value_t = 300, help_heading = "Fault-Driven Log")]
    fault_log_ttl_secs: u64,
...
/// Best-effort host identifier for fault-log filenames + notify bodies.
/// Falls back to "unknown" so log lines never explode mid-startup.
...
Provide --worker-urls or PD flags as usual.",
    ...
    // Initialize the fault-driven log subsystem BEFORE starting the
    // server so the first dispatched request already has the room
    // buffer + notify endpoint wired up. When --enable-fault-driven-log
    // is not set, init() still runs but writes an `enabled=false`
    // state, making every emit / notify a cheap early return.
    {
        use std::path::PathBuf;
        use std::time::Duration;

        use smg::observability::fault_log::{self, FaultLogConfig};

        let self_url = if !cli_args.fault_log_self_url.trim().is_empty() {
            Some(cli_args.fault_log_self_url.trim().to_string())
        } else if cli_args.enable_fault_driven_log {
            Some(format!(
                "http://{}:{}",
                if cli_args.host.is_empty() {
                    "127.0.0.1"
                } else {
                    cli_args.host.as_str()
                },
                cli_args.port
            ))
        } else {
        ...
        let token = if cli_args.fault_log_token.trim().is_empty() {
            None
        } else {
            Some(cli_args.fault_log_token.trim().to_string())
        };
        let cfg = FaultLogConfig {
            enabled: cli_args.enable_fault_driven_log,
            dir: PathBuf::from(&cli_args.fault_log_dir),
            token,
            source_label: format!("router@{}", hostname_or_default()),
            max_rooms: cli_args.fault_log_max_rooms,
            max_events_per_room: cli_args.fault_log_max_events_per_room,
            ttl: Duration::from_secs(cli_args.fault_log_ttl_secs),
            error_whitelist: cli_args.fault_log_error_whitelist.clone(),
