...
use crate::{
    ...
    observability::{
        events::{self, Event},
        fault_log::{self, ROUTER_URL_HEADER},
...
impl PDRouter {
    ...
    /// Best-effort extraction of `bootstrap_room` from the post-inject body.
    ///
    /// The inject path can put either a single u64 or a list of u64s
    /// into the JSON depending on whether the request was batched.
    /// For batched requests we return the FIRST element so peer-notify
    /// has a stable join key — peers can still locate the rest via the
    /// buffer when offline reconstructing.
    fn extract_bootstrap_room(json: &Value) -> Option<u64> {
        let v = json.get(Self::BOOTSTRAP_ROOM_KEY)?;
        if let Some(n) = v.as_u64() {
            return Some(n);
        }
        if let Some(arr) = v.as_array() {
            return arr.first().and_then(|x| x.as_u64());
        }
        None
    }
            ...
            &self.retry_config,
            {
                move |attempt: u32| {
                    ...
                    async move {
                        ...
                        // Fault-driven log: capture the dispatch decision in
                        // the per-room buffer + register the Python peers so
                        // they receive subsequent /fault-log/notify if the
                        // router observes an error after streaming starts.
                        // Cheap no-op when the feature is disabled.
                        if let Some(state) = fault_log::global() {
                            if state.config.enabled {
                                if let Some(room) =
                                    Self::extract_bootstrap_room(&json_request)
                                {
                                    state.add_peer(prefill.url());
                                    state.add_peer(decode.url());
                                    state.emit(
                                        room,
                                        "dispatch",
                                        json!({
    ...
    async fn execute_dual_dispatch_internal(
        ...
        // Tell the downstream Python servers where to send their
        // /fault-log/notify requests. The router's own listening
        // address is recorded on fault-log init; we read it from the
        // global state. No-op when the feature is disabled.
        if let Some(state) = fault_log::global() {
            if state.config.enabled {
                if let Some(self_url) = state.self_url() {
                    if let Ok(hv) = HeaderValue::from_str(&self_url) {
