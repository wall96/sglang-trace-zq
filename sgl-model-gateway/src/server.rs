...
/// Inbound /fault-log/notify endpoint.
///
/// Body shape (JSON)::
///
///     {
...
///         "ts": <f64>,
///         "extra": { "rid": <str>, "status_code": <u16>, ... }
///     }
///
/// The peer's token is sent in the X-Sgl-Fault-Log-Token header. When
/// fault-driven logging is disabled on this router, we accept and
/// silently discard so partial deployments don't blacklist us.
async fn fault_log_notify(headers: HeaderMap, body: axum::body::Bytes) -> Response {
    use crate::observability::fault_log;
    let Some(state) = fault_log::global() else {
        return (StatusCode::OK, "fault-log feature not initialised").into_response();
    };
    let token_header = headers
        .get(fault_log::FAULT_LOG_TOKEN_HEADER)
        .and_then(|v| v.to_str().ok());
    let parsed: Result<fault_log::InboundNotify, _> = serde_json::from_slice(&body);
    let Ok(notify) = parsed else {
        return (StatusCode::BAD_REQUEST, "invalid json").into_response();
    };
    match fault_log::handle_inbound_notify(&state, token_header, notify).await {
        Ok(_) => (StatusCode::OK, "ok").into_response(),
        Err((code, msg)) => {
            let sc = StatusCode::from_u16(code).unwrap_or(StatusCode::BAD_REQUEST);
            (sc, msg).into_response()
        }
    }
}
...
pub fn build_app(
    ...
    let public_routes = Router::new()
        ...
        // Cross-component fault-log notification endpoint. The Python
        // prefill / decode servers call this when they observe an error
        // in a request they handled, so the router can dump its own
        // local view of that bootstrap_room. Auth is via a shared token
        // header (X-Sgl-Fault-Log-Token); see fault_log.rs.
        .route("/fault-log/notify", post(fault_log_notify));
