...
def prepare_abort(req: Req, error_message: str, status_code=None):
    ...
    # Emit a path event capturing the abort site BEFORE setting the finish
    # reason, so the call-site filename/line is preserved for debugging.
    # This is the single funnel that all disagg-side aborts go through
    # (input length exceeded, KVPoll.Failed in bootstrap/prealloc/inflight/
    # transfer queues, etc.), so a single emit here automatically covers
    # every disagg error site without per-site instrumentation.
    # Cost on success path is zero — prepare_abort is only invoked on errors.
    ...
    try:
        ...
        f = sys._getframe(1)
        caller = f"{f.f_code.co_filename.rsplit('/', 1)[-1]}:{f.f_lineno}"
    except Exception:
        pass
    if time_stats is not None and hasattr(time_stats, "emit"):
        try:
            time_stats.emit(
                "abort",
                message=str(error_message)[:512],
            ...
            # Logging must never break abort.
            pass

    # Note: ``prepare_abort`` runs in the scheduler process, NOT the
    # tokenizer_manager process where the room buffer lives. The
    # ``req.time_stats.emit`` above is the right path: scheduler events
    # ride back to tokenizer_manager via BatchTokenIDOutput.time_stats
    # pickling. We do NOT touch a global room buffer here because there
    # is none in the scheduler process.
