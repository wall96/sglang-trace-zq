...
class ServerArgs:
    ...
    # Fault-driven per-request logging: when enabled, records every request's
    # path through emit() events and dumps the full timeline + error context
    # to disk only when the request finishes with an error. When disabled,
    # behavior is identical to upstream sglang (no extra cost on hot path).
    enable_fault_driven_log: bool = False
    fault_log_dir: str = "/tmp/sglang_fault_log"
    # Whitelist of error message substrings or status codes that gate which
    # failed requests get persisted. None / empty list = persist all errors.
    # Examples: ["bootstrap timeout", "KVPoll.Failed", "400", "500"].
    fault_log_error_whitelist: Optional[List[str]] = None
    # Cross-component fault log notify config. When a request errors on
    # this server, an async HTTP POST is fired to peer servers (router,
    # the other side of PD disagg) via /fault-log/notify so they dump
    # their local view of the same bootstrap_room. The token gates the
    # endpoint against accidental cross-traffic / DoS.
    fault_log_token: Optional[str] = None
    fault_log_router_url: Optional[str] = None
    # Per-room in-memory ring buffer sizing. The buffer outlives the Req
    # object and is the source of events when peer-notify arrives after
    # this server has already finished its part of the request.
    fault_log_room_buffer_max_rooms: int = 5000
    fault_log_room_buffer_max_events_per_room: int = 200
    fault_log_room_buffer_ttl_secs: float = 300.0
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=ServerArgs.enable_fault_driven_log,
            help=(
                "Enable per-request fault-driven logging. When enabled, every "
                "request's path through the pipeline is recorded into "
                "req.time_stats.events; if the request finishes with an error "
                "(FINISH_ABORT or HTTP >= 400), the full timeline + error "
                "context is dumped asynchronously to --fault-log-dir. "
                "Successful requests have only an in-memory append cost and "
                "are never persisted. Off by default; behavior is identical "
                "to upstream sglang when not set."
            ),
        )
        parser.add_argument(
            "--fault-log-dir",
            type=str,
            default=ServerArgs.fault_log_dir,
            help="Directory where fault-driven logs are persisted (one pkl per failed request).",
        )
        parser.add_argument(
            "--fault-log-error-whitelist",
            type=str,
            nargs="*",
            default=ServerArgs.fault_log_error_whitelist,
            help=(
                "Whitelist of error message substrings or HTTP status codes "
                "that gate which failed requests are dumped. Match is "
                "case-insensitive substring against the abort message and "
                "exact-match against the status code string. Omit / empty to "
                "dump all errors. Examples: --fault-log-error-whitelist "
                "'bootstrap timeout' 'KVPoll.Failed' 500"
            ),
        )
        parser.add_argument(
            "--fault-log-token",
            type=str,
            default=ServerArgs.fault_log_token,
            help=(
                "Shared secret used to authenticate /fault-log/notify "
                "requests across router/prefill/decode. All cooperating "
                "components must use the same token. When --enable-fault-"
                "driven-log is set and this is left empty, peer notifies "
                "are skipped (local dumps still happen)."
            ),
        )
        parser.add_argument(
            "--fault-log-router-url",
            type=str,
            default=ServerArgs.fault_log_router_url,
            help=(
                "Base URL of the router's /fault-log/notify endpoint. "
                "Used by prefill/decode to push error notifications back to "
                "the router. If unset, prefill/decode try to recover the URL "
                "from the X-Sgl-Router-Fault-Log-Url request header that "
                "the router injects at dispatch time. Falling back to local "
                "dump only when neither is available."
            ),
        )
        parser.add_argument(
            "--fault-log-room-buffer-max-rooms",
            type=int,
            default=ServerArgs.fault_log_room_buffer_max_rooms,
            help="Cap on rooms tracked in the per-process fault-log ring buffer.",
        )
        parser.add_argument(
            "--fault-log-room-buffer-max-events-per-room",
            type=int,
            default=ServerArgs.fault_log_room_buffer_max_events_per_room,
            help="Cap on events kept per room in the fault-log ring buffer.",
        )
        parser.add_argument(
            "--fault-log-room-buffer-ttl-secs",
            type=float,
            default=ServerArgs.fault_log_room_buffer_ttl_secs,
            help="Idle TTL (seconds) before a room is GC'd from the fault-log buffer.",
        )
        parser.add_argument(
            "--kv-events-config",
            type=str,
