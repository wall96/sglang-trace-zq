...
def _maybe_register_router_peer_from_headers(request: Request) -> None:
    """Pick up the router's fault-log URL from request headers.

    The router injects ``X-Sgl-Router-Fault-Log-Url`` on every dispatch
    when fault-driven logging is enabled cluster-wide. Prefill / decode
    pick that up here and register it as a peer with their notifier so
    that subsequent failed requests get a notify back to the router.

    Cheap no-op when the feature is disabled (notifier is None).
    """
    tm = _global_state.tokenizer_manager
    if not getattr(tm, "enable_fault_driven_log", False):
        return
    register = getattr(tm, "fault_log_register_peer", None)
    if register is None:
        return
    try:
        url = request.headers.get("x-sgl-router-fault-log-url") or request.headers.get(
            "X-Sgl-Router-Fault-Log-Url"
        )
        if url:
            register(url)
    except Exception:
        # Header parsing must never break a generate request.
        pass
...
async def generate_request(obj: GenerateReqInput, request: Request):
    ...
    _maybe_register_router_peer_from_headers(request)
    if obj.stream:
...
@app.api_route("/fault-log/notify", methods=["POST"])
async def fault_log_notify(request: Request):
    """Cross-component fault-log notification endpoint.

    Peers (router / the other side of PD disagg) call this when they
    write a fault-log entry for a request, so this server can dump
    its own local view of the same ``bootstrap_room``.

    Auth: ``X-Sgl-Fault-Log-Token`` header must match the configured
    token (when set). Returns 401 on mismatch.
    ...
    Body schema (JSON)::

        {
            ...
            "error_summary": <str>,
            "source": <str>,              # peer's component label
            "local_dump_path": <str|null>,
            "ts": <float>,                # unix time at peer
            "extra": { "rid": <str>, "status_code": <int>, ... }
        }

    Always returns 200 when the feature is disabled on this server,
    so peers don't blacklist us during partial deployments. When the
    feature is enabled, returns 401 / 400 on bad input.
    """
    tm = _global_state.tokenizer_manager
    handler = getattr(tm, "handle_fault_log_notify", None)
    if handler is None:
        return Response(
            content="fault-log feature not present in this server build",
            status_code=200,
        )
    token_header = request.headers.get(
        "x-sgl-fault-log-token"
    ) or request.headers.get("X-Sgl-Fault-Log-Token")
    try:
        body = await request.json()
    except Exception as e:
        return Response(content=f"invalid json: {e!r}", status_code=400)
    ok, msg = handler(body, token_header)
    if not ok:
        if msg == "auth":
