...
class TokenizerManager(TokenizerControlMixin, TokenizerManagerScoreMixin):
    ...
    def init_request_logging_and_dumping(self):
        ...
        # Fault-driven per-request logging.
        # When enable_fault_driven_log is False (default), all the helpers
        # below short-circuit and the system behaves identically to upstream.
        self.enable_fault_driven_log: bool = bool(
            getattr(server_args, "enable_fault_driven_log", False)
        )
        self.fault_log_dir: str = (
            getattr(server_args, "fault_log_dir", "") or "/tmp/sglang_fault_log"
        )
        # Pre-normalize the whitelist for cheap matching on the hot finish
        # path. Each entry is matched as a case-insensitive substring against
        # the abort message AND as an exact-string match against the status
        # code (when present). None / empty list = match everything.
        raw_wl = getattr(server_args, "fault_log_error_whitelist", None) or []
        self.fault_log_error_whitelist: List[str] = [
            str(s).strip() for s in raw_wl if str(s).strip()
        ]
        self._fault_log_wl_lower: List[str] = [
            s.lower() for s in self.fault_log_error_whitelist
        ]
        # Per-room ring buffer + cross-component notifier. Both are
        # constructed eagerly even when the feature is off so external
        # callers (e.g. mooncake bg threads) can do the cheap
        # ``self.fault_log_room_buffer.emit(room, ...)`` call
        # unconditionally, and we only pay the cost when the feature is
        # actually enabled (the buffer's emit is itself ~200ns and
        # caps at zero work when disabled by short-circuit at call site).
        self.fault_log_room_buffer = None
        self.fault_log_notifier = None
        if self.enable_fault_driven_log:
            try:
                from sglang.srt.observability.room_event_buffer import (
                    RoomEventBuffer,
                )
                from sglang.srt.observability.fault_log_notifier import (
                    FaultLogNotifier,
                )

                self.fault_log_room_buffer = RoomEventBuffer(
                    max_rooms=getattr(
                        server_args, "fault_log_room_buffer_max_rooms", 5000
                    ),
                    max_events_per_room=getattr(
                        server_args,
                        "fault_log_room_buffer_max_events_per_room",
                        200,
                    ),
                    ttl_secs=getattr(
                        server_args, "fault_log_room_buffer_ttl_secs", 300.0
                    ),
                )
                disagg_mode = getattr(server_args, "disaggregation_mode", None)
                source_label = (
                    f"{disagg_mode or 'unified'}@{socket.gethostname()}"
                )
                self.fault_log_notifier = FaultLogNotifier(
                    token=getattr(server_args, "fault_log_token", None),
                    source_label=source_label,
                )
                # Pre-register the router URL if configured statically.
                router_url = getattr(server_args, "fault_log_router_url", None)
                if router_url:
                    self.fault_log_notifier.add_peer(router_url)
                logger.info(
                    "fault-driven log enabled: dir=%s, "
                    "whitelist=%s, token=%s, router_peer=%s",
                    self.fault_log_dir,
                    self.fault_log_error_whitelist or "<all errors>",
                    "set" if getattr(server_args, "fault_log_token", None) else "unset",
                    router_url or "<via header>",
                )
            except Exception as e:
                # Don't crash startup if observability deps are missing.
                logger.warning(
                    "Failed to init fault-driven log infra: %r. "
                    "Falling back to local-only dump (no peer notify).",
                    e,
                )
    ...
    def _send_one_request(
        ...
        # Fault-driven log: record api_dispatch in the per-room buffer so
        # that a later /fault-log/notify (e.g. router observed an error
        # streaming back) can recover this server's view of when it sent
        # the request to the scheduler. No-op when feature is off.
        if self.fault_log_room_buffer is not None:
            try:
                room = getattr(tokenized_obj, "bootstrap_room", None)
                rid = getattr(tokenized_obj, "rid", None)
                self.fault_log_room_buffer.emit(
                    room,
                    "api_dispatch",
                    rid=rid,
                )
    ...
    async def _handle_batch_output(
        ...
        for i, rid in enumerate(recv_obj.rids):
            ...
            # Fault-driven per-request logging: only persists when the request
            # finished with an error AND the error matches the optional
            # whitelist. When the feature is off, this is a single attribute
            # read (`self.enable_fault_driven_log`) and a short-circuit.
            if (
                self.enable_fault_driven_log
                and state.finished
                and self._should_dump_for_fault(state, out_dict, recv_obj, i)
            ):
                # Emit an api-side abort marker into the room buffer so a
                # peer-notify arriving for this room later (after the local
                # state is gone) can still see that this server observed
                # the failure.
                if self.fault_log_room_buffer is not None:
                    try:
                        err = self._extract_finish_error(out_dict, recv_obj, i)
                        msg, sc = err if err else ("", None)
                        self.fault_log_room_buffer.emit(
                            getattr(state.obj, "bootstrap_room", None),
                            "api_abort",
                            rid=getattr(state.obj, "rid", None),
                            message=str(msg)[:512],
                            status_code=sc,
                        )
                    except Exception:
                        pass
                self.dump_failed_request(state, out_dict, recv_obj, i)
    ...
    def _extract_finish_error(
        self, out_dict: dict, recv_obj, idx: int
    ) -> Optional[Tuple[str, Optional[int]]]:
        """Return (message, status_code) if this request finished with an
        error, else None.

        We treat a request as "errored" when EITHER:
          * its finish_reason is FINISH_ABORT (FINISH_ABORT serializes to
            ``{"type": "abort", "message": ..., "status_code": ...}``), OR
          * the recv_obj's per-index finished_reasons entry indicates abort
            via the same shape.
        """
        # Fast path: out_dict["meta_info"]["finish_reason"] is the canonical
        # serialized form that flows out to the client.
        meta = out_dict.get("meta_info") if isinstance(out_dict, dict) else None
        ...
        # Fallback: inspect recv_obj.finished_reasons[idx]
        try:
            fr2 = recv_obj.finished_reasons[idx]
            if isinstance(fr2, dict) and fr2.get("type") == "abort":
                return (str(fr2.get("message", "")), fr2.get("status_code"))
        except (AttributeError, IndexError, TypeError):
            pass
        return None

    def _matches_error_whitelist(
        self, message: str, status_code: Optional[int]
    ) -> bool:
        """Apply the user's error whitelist.

        Empty whitelist => match all errors. Otherwise we match
        case-insensitive substring against the error message AND
        exact-string against the status_code (so users can write '500',
        '400', or 'bootstrap timeout', etc.).
        """
        if not self._fault_log_wl_lower:
            return True  # default: capture every error
        msg_lower = (message or "").lower()
        status_str = str(status_code) if status_code is not None else ""
        for pat_lower, pat_raw in zip(
            self._fault_log_wl_lower, self.fault_log_error_whitelist
        ):
            if pat_lower and pat_lower in msg_lower:
                return True
            if status_str and pat_raw == status_str:
                return True
        return False

    def _should_dump_for_fault(
        self, state: ReqState, out_dict: dict, recv_obj, idx: int
    ) -> bool:
        err = self._extract_finish_error(out_dict, recv_obj, idx)
        if err is None:
            return False
        message, status_code = err
        return self._matches_error_whitelist(message, status_code)

    def _fault_log_source_label(self) -> str:
        """Human-readable component tag used in dump filenames + notifies."""
        disagg = getattr(self.server_args, "disaggregation_mode", None) or "unified"
        return str(disagg)

    def dump_failed_request(
        self, state: ReqState, out_dict: dict, recv_obj, idx: int
    ) -> None:
        """Dump a single failed request asynchronously to the fault-log dir.

        Layout: ``<fault_log_dir>/<YYYYMMDD>/<source>__<bootstrap_room>__<rid>.pkl``

        The ``<bootstrap_room>`` segment is the join key — pkl files
        produced on different servers (router/prefill/decode) for the
        same request all share it, so an offline tool can ``ls`` the
        directory to reconstruct the cross-component timeline.

        Each pkl contains a self-describing dict with: rid, bootstrap_room,
        host, finish_reason, error_message, status_code, full event timeline
        merged from API-server-side, scheduler-side, AND room-buffer events
        (the last picks up mooncake background-thread events keyed by
        bootstrap_room). After writing, an async fire-and-forget HTTP
        notify is sent to each registered peer (router and the other side
        of PD disagg) so they dump their local view too.
        """
        # Strip heavy keys (routed_experts, hidden_states) from meta_info,
        # mirroring dump_requests' policy. We never modify the original
        # out_dict that was already streamed to the client.
        clean_out_dict = out_dict
        if self.dump_requests_exclude_meta_keys and isinstance(
            ...
            if any(k in out_dict["meta_info"] for k in exclude):
                ...
                clean_out_dict = {**out_dict, "meta_info": filtered_meta}

        # Pull the scheduler-side time_stats (carries the events list that
        # was emitted in the scheduler / disagg / mooncake processes).
        scheduler_time_stats = None
        try:
            if recv_obj.time_stats is not None:
                scheduler_time_stats = recv_obj.time_stats[idx]
        except (AttributeError, IndexError, TypeError):
            scheduler_time_stats = None

        api_events = list(getattr(state.time_stats, "events", []) or [])
        sched_events = list(getattr(scheduler_time_stats, "events", []) or [])
        # Pull mooncake / disagg background-thread events keyed by room.
        room = getattr(state.obj, "bootstrap_room", None)
        room_events = []
        if self.fault_log_room_buffer is not None:
            try:
                # ``drain`` (instead of ``get``) so the room is freed
                # from the buffer right after we capture its history —
                # keeps memory bounded under high error rates.
                room_events = self.fault_log_room_buffer.drain(room)
            except Exception:
                room_events = []

        # Tag with origin for unambiguous replay.
        merged_events: List[Tuple[str, str, float, dict]] = []
        for stage, ts, attrs in api_events:
            merged_events.append(("api_server", stage, ts, attrs))
        for stage, ts, attrs in sched_events:
            merged_events.append(("scheduler", stage, ts, attrs))
        for stage, ts, attrs in room_events:
            merged_events.append(("room_buffer", stage, ts, attrs))
        merged_events.sort(key=lambda e: e[2])

        err = self._extract_finish_error(clean_out_dict, recv_obj, idx) or (
            "",
            None,
        )
        message, status_code = err

        rid = getattr(state.obj, "rid", None) or "unknown"
        if isinstance(rid, list) and rid:
            rid = rid[0]
        # Sanitize rid for filesystem use.
        safe_rid = "".join(
            c if c.isalnum() or c in ("-", "_", ".") else "_" for c in str(rid)
        )
        source_tag = self._fault_log_source_label()
        room_tag = str(room) if room is not None else "noroom"
        ...
        payload = {
            ...
            "pid": os.getpid(),
            "bootstrap_room": room,
            "disagg_mode": getattr(self.server_args, "disaggregation_mode", None),
            "source": source_tag,
            "obj": state.obj,
            "out_dict": clean_out_dict,
            "finish_reason": (
                clean_out_dict.get("meta_info", {}).get("finish_reason")
                if isinstance(clean_out_dict, dict)
                else None
            ),
            "error_message": message,
            "status_code": status_code,
            "created_time": convert_time_to_realtime(state.time_stats.created_time),
            "finished_time": convert_time_to_realtime(state.time_stats.finished_time),
            # [(origin, stage, perf_counter_ts, attrs), ...]
            "events": merged_events,
            "api_time_stats": state.time_stats,
            "scheduler_time_stats": scheduler_time_stats,
            "notified_by": None,  # set when this dump was triggered by a peer notify
        }

        sub = os.path.join(
            self.fault_log_dir, datetime.now().strftime("%Y%m%d")
        )
        filename = os.path.join(
            sub, f"{source_tag}__{room_tag}__{safe_rid}.pkl"
        )
        # Reuse the existing async file dumper. It already handles directory
        # creation and pickle fallbacks and runs on asyncio.to_thread so the
        # tokenizer event loop doesn't stall on disk.
        self._dump_data_to_file(
            data_list=[payload],
            filename=filename,
            log_message=(
                f"[fault-log] persisted failed request rid={rid} "
                f"room={room} status={status_code} -> {filename}"
            ),
        )

        # Fire-and-forget peer notification so the router (and the other
        # side of PD disagg) also dumps their view of this room. Notifier
        # may be None when init failed; in that case we silently skip
        # notify and only the local dump is produced (the bootstrap_room
        # in the filename still allows offline join).
        if (
            self.fault_log_notifier is not None
            and room is not None
            and self.fault_log_notifier.is_active()
        ):
            try:
                self.fault_log_notifier.notify_room_failure(
                    room=room,
                    error_summary=str(message)[:512],
                    local_dump_path=filename,
                    extra={
                        "rid": rid,
                        "status_code": status_code,
                        "source": source_tag,
                ...
                logger.warning(
                    "fault-log notify dispatch failed for room=%s: %r", room, e
                )

    # ------------------------------------------------------------------ #

    def fault_log_register_peer(self, url: Optional[str]) -> Optional[str]:
        """Idempotently register a peer URL with the notifier.

        Called from places that learn about a new peer at runtime —
        e.g. when an incoming HTTP request carries the
        ``X-Sgl-Router-Fault-Log-Url`` header injected by the router on
        dispatch, or when mooncake registration tells prefill where
        decode's HTTP fault-log endpoint lives.
        """
        if self.fault_log_notifier is None or not url:
            return None
        return self.fault_log_notifier.add_peer(url)

    def handle_fault_log_notify(
        self, body: Dict[str, Any], request_token: Optional[str]
    ) -> Tuple[bool, str]:
        """Handle an inbound /fault-log/notify request.

        Returns ``(ok, message)``. Caller (HTTP layer) maps ``ok=False``
        to HTTP 401 (auth) or 400 (bad payload); ``ok=True`` -> 200.
        """
        if not self.enable_fault_driven_log or self.fault_log_room_buffer is None:
            # Feature is off — politely accept and discard so peers don't
            # blacklist us mid-deployment when configs differ briefly.
            return True, "fault-log feature is disabled on this server"
        if self.fault_log_notifier is not None and not self.fault_log_notifier.authenticate(
            request_token
        ):
            return False, "auth"
        ...
        # Pull the local view of this room (events emitted on this
        # server before/around the time the peer hit the error).
        try:
            local_events = self.fault_log_room_buffer.drain(room)
        except Exception:
            local_events = []

        # Even when we have no local events we still write a stub file
        # — that fact alone is informative for offline join ("server X
        # never saw room R" => router error before dispatch, or
        # request never made it to this side).
        merged_events: List[Tuple[str, str, float, dict]] = [
            ("room_buffer", stage, ts, attrs)
            for (stage, ts, attrs) in local_events
        ]
        source_tag = self._fault_log_source_label()
        peer_source = body.get("source") or "unknown_peer"
        peer_dump_path = body.get("local_dump_path")
        ts_now = real_time()

        payload = {
            "rid": (body.get("extra") or {}).get("rid"),
            "host": socket.gethostname(),
            "pid": os.getpid(),
            "bootstrap_room": room,
            "disagg_mode": getattr(self.server_args, "disaggregation_mode", None),
            "source": source_tag,
            "obj": None,  # we don't have the original obj here
            "out_dict": None,
            "finish_reason": None,
            "error_message": body.get("error_summary"),
            "status_code": (body.get("extra") or {}).get("status_code"),
            "created_time": None,
            "finished_time": ts_now,
            "events": merged_events,
            "api_time_stats": None,
            "scheduler_time_stats": None,
            "notified_by": {
                "peer_source": peer_source,
                "peer_dump_path": peer_dump_path,
                "received_at": ts_now,
            },
        }
        ...
        filename = os.path.join(
            sub,
            f"{source_tag}__{room}__notified_by_{peer_source.split('@')[0]}.pkl",
        )
        try:
            self._dump_data_to_file(
                data_list=[payload],
                filename=filename,
                log_message=(
                    f"[fault-log] peer-notify dump room={room} "
                    f"from={peer_source} -> {filename}"
                ),
            )
        except Exception as e:
            logger.warning(
                "Failed to write peer-notify dump for room=%s: %r", room, e
            )
            return False, f"dump_failed: {e!r}"
        return True, "ok"
