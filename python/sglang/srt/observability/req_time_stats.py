...
class ReqTimeStatsBase:
    ...
    # Per-request event log used by the fault-driven logging feature.
    # Each entry is (stage_name, perf_counter_timestamp, attrs_dict).
    # When the fault-driven log feature is disabled (default), nothing
    # appends to this list, so overhead is exactly zero.
    events: List[Any] = field(default_factory=list)

    def emit(self, stage: str, **attrs: Any) -> None:
        """Append a stage event to this request's path log.

        Hot path: a single perf_counter() call + list.append. No locks,
        no serialization, no I/O. Safe to call from any thread that
        owns this Req's time_stats. The OpenTelemetry trace_ctx (if
        enabled) gets a parallel trace_event so OTel users can see
        the same events in their backend.
        """
        ts = time.perf_counter()
        self.events.append((stage, ts, attrs))
    ...
    def __getstate__(self) -> object:
        ...
        # Always propagate events list (empty by default, so overhead is
        # negligible when fault-driven logging is off). When the feature is
        # on, this carries the full path from the producing process to the
        # tokenizer_manager that does the dump.
        if self.events:
...
class SchedulerReqTimeStats(ReqTimeStatsBase):
    ...
    def __getstate__(self) -> object:
        ...
        # Note: even when enable_metrics is False, we still propagate the
        # fault-driven log events list (when non-empty) so the receiving
        # tokenizer_manager can persist the full path on error. This
        # short-circuits cheaply when no emit() has been called.
        state: Dict[str, Any] = {}
        if self.events:
            state["events"] = self.events
            # Always carry diff so __setstate__'s key.endswith("time")
            # path doesn't crash if metrics are disabled but events exist.
